...
export async function PATCH(req: NextRequest, {params}:{params:Parametro}){
    try {
        ...
        const ordemAtualizada = await OrdemServicoService.updateOrdem(id, data);
        if(!ordemAtualizada){
...
export async function GET ({params}:{params:Parametro}){
    try {
        const {id} = params;
        const data = await OrdemServicoService.getOrdemById(id);
        if(!data){
            return NextResponse.json({success:false, error: "Not Found"});
        }
...
export async function DELETE({params}:{params:Parametro}) {
    try {
        const {id} = params;
        await OrdemServicoService.deleteOrdem(id);
